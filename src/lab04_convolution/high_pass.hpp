...
[[nodiscard]] Kernel2D make_laplacian_kernel(LaplacianKernel kernel, std::size_t size = 3);
[[nodiscard]] Kernel2D make_log_kernel(std::size_t size, double sigma);
[[nodiscard]] Kernel2D make_sharpening_kernel(std::size_t size = 5, double amount = 1.0);
...
[[nodiscard]] GrayImage laplacian_filter(const GrayImage& image, LaplacianKernel kernel, std::size_t kernel_size = 3);
...
[[nodiscard]] GrayImage sharpening_filter(const GrayImage& image, std::size_t kernel_size = 5, double amount = 1.0);
