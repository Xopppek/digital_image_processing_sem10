...
сохраняется количество найденных областей, площадь и коэффициент округлости
`Kr2 = 2 * pi * (mu20 + mu02) / S^2`, где `S` - площадь области. Для круга
значение `Kr2` близко к `1`.

### Домашнее задание

Первый этап домашнего задания - ручное чтение несжатых TIFF-файлов с
полутоновыми изображениями:

```sh
./build/dip homework read-tiff --input path/to/input.tif --output path/to/output.png
```

Поддерживаются TIFF-файлы без сжатия, с 8-битным полутоновым или RGB
изображением, порядком байтов Intel или Motorola и одним или несколькими strip.
Графический файл разбирается вручную; готовые функции чтения изображений для
этого шага не используются. Команда `read-tiff` сохраняет RGB TIFF как цветное
изображение. Для основного пайплайна RGB TIFF вручную переводится в
полутоновый `GrayImage`. Дополнительные примеры для проверки только ручной
читалки лежат в `images/homework/input/tif_reader`; результаты сохраняются в
`images/homework/output/tif_reader` без выделения блоков и восстановления.

Разделить скан страницы на блоки и подсветить найденные области изображения:

```sh
./build/dip homework classify-blocks --input images/homework/input/3.tif --output images/homework/output/3_blocks.png --block-size 56 --metrics-output images/homework/output/3_blocks.json
```

В результате блоки изображения подсвечиваются красным цветом, а фон и текст
остаются без подсветки. Классификация использует простые признаки по
гистограмме блока: долю белых, небелых, темных и полутоновых пикселов.
После первичной классификации соседние блоки изображения объединяются в
4-связные области; маленькие области и блоки без соседней поддержки
исключаются из класса изображения. Параметры постобработки можно менять через
`--min-region-blocks` и `--min-neighbors`. Точные значения признаков, итоговый
класс каждого блока и найденные области сохраняются в JSON.

Запустить восстановление растрированной фотографии на странице:

```sh
./build/dip homework restore-halftone --input images/homework/input/3.tif --output images/homework/output/3_restored.png --blurred-output images/homework/output/3_blurred.png --blocks-output images/homework/output/3_restore_blocks.png --metrics-output images/homework/output/3_restore.json
```

Пайплайн читает TIFF, выделяет блоки изображения, не использует ячейки,
похожие на текст по гистограмме и горизонтальным проекциям, при построении
объемлющих прямоугольников и расширяет найденные прямоугольники примерно на
пятую часть размера ячейки. Затем эти прямоугольники размываются ручным
гауссовым фильтром, после чего повышается четкость с помощью лапласиана.
Текст вне найденных прямоугольников остается без обработки.
Промежуточное изображение после размытия можно сохранить через
`--blurred-output`.
По умолчанию области меньше 12 блоков отбрасываются, чтобы плотные строки
текста не попадали в обработку как небольшие изображения. Для подавления
растровой текстуры размер ядра должен быть достаточно
большим относительно шага растра; для текущего примера по умолчанию
используется ядро `11x11` с `sigma = 1.8` и лапласианное усиление резкости
`--sharpen-amount 0.35`. Параметры фильтров можно менять через
`--gaussian-kernel-size`, `--gaussian-sigma` и `--sharpen-amount`, а запас
вокруг прямоугольников - через `--rectangle-padding-fraction`.
...
- для домашнего задания читает TIFF-файлы из `images/homework/input`, отдельно
  читает проверочные файлы из `images/homework/input/tif_reader` и сохраняет
  прочитанные изображения, цветную разметку блоков, восстановленные изображения
  и JSON с признаками в `images/homework/output`. Полный пайплайн восстановления
  запускается для всех TIFF-файлов верхнего уровня в `images/homework/input`.
...
scripts/run_all_examples.sh homework
```
