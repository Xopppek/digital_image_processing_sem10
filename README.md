...
Поддерживаются TIFF-файлы без сжатия, с 8-битным полутоновым изображением,
порядком байтов Intel или Motorola и одним или несколькими strip. Графический
файл разбирается вручную; готовые функции чтения изображений для этого шага не
используются.

Разделить скан страницы на блоки и подсветить найденные области текста и
изображения:

```sh
./build/dip homework classify-blocks --input images/homework/input/3.tif --output images/homework/output/3_blocks.png --block-size 64 --metrics-output images/homework/output/3_blocks.json
```

В результате текстовые блоки подсвечиваются зеленым цветом, блоки изображения -
красным, фон остается без подсветки. Классификация использует простые признаки
по гистограмме блока: долю белых, небелых, темных и полутоновых пикселов.
...
- для домашнего задания читает TIFF-файлы из `images/homework/input` и сохраняет
  прочитанные изображения, цветную разметку блоков и JSON с признаками в
  `images/homework/output`.
